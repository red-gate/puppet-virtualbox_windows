...
class virtualbox_windows::hostonly_network_adapter(
  ...
  exec { 'Create VirtualBox host only adapter':
    command => "& \"C:/Program Files/Oracle/VirtualBox/VBoxManage.exe\" hostonlyif create",
    onlyif  => template('virtualbox_windows/should_create_adapter.ps1.erb'),
  }
  ->
  exec { 'Virtualbox: vboxmanage dhcpserver add' :
    command => "& \"C:/Program Files/Oracle/VirtualBox/VBoxManage.exe\" dhcpserver add --netname \"${dhcp_netname}\" --ip ${dhcp_ipaddress} --netmask 255.255.255.0 --lowerip ${dhcp_lower_ip_range} --upperip ${dhcp_upper_ip_range} --enable",
    onlyif  => template('virtualbox_windows/should_create_dhcp_server.ps1.erb'),
  }
  ->
  exec { 'Virtualbox: vboxmanage dhcpserver modify' :
    command => "& \"C:/Program Files/Oracle/VirtualBox/VBoxManage.exe\" dhcpserver modify --netname \"${dhcp_netname}\" --ip ${dhcp_ipaddress} --netmask 255.255.255.0 --lowerip ${dhcp_lower_ip_range} --upperip ${dhcp_upper_ip_range} --enable",
  ...
  exec { "Set VirtualBox Host-Only Ethernet Adapter IP to ${adapter_idaddress}" :
    command => "& \"C:/Program Files/Oracle/VirtualBox/VBoxManage.exe\" hostonlyif ipconfig \"VirtualBox Host-Only Ethernet Adapter\" --ip ${adapter_idaddress}",
  ...
  exec { 'Set VirtualBox Host-Only Ethernet Adapter DHCP' :
    command     => "& \"C:/Program Files/Oracle/VirtualBox/VBoxManage.exe\" hostonlyif ipconfig \"VirtualBox Host-Only Ethernet Adapter\" --dhcp",
    refreshonly => true,
